...
function EmptyState({ documents, selectedIds, onSuggest }) {
  ...
  const suggestions  = ['Summarize this document', 'What are the key concepts?', 'List the main topics']

  if (documents.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-2 text-center">
        <p className="text-sm text-slate-500">Upload a document to get started</p>
        <p className="text-xs text-slate-400">Use the sidebar to upload PDFs, text files, or images</p>
      </div>
    )
  }
      ...
      {selectedDocs.length > 0 && (
        <div className="w-full space-y-3">
          {selectedDocs.map((doc) => (
            <div key={doc.document_id} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="ml-auto text-xs text-slate-400 shrink-0">{doc.total_chunks} chunks</span>
              </div>
              {doc.summary && (
                <p className="text-xs leading-relaxed text-slate-500 line-clamp-3">{doc.summary}</p>
              )}
              {doc.key_topics?.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {doc.key_topics.slice(0, 5).map((t) => (
                    <span key={t} className="rounded-full bg-white border border-slate-200 px-2 py-0.5 text-xs text-slate-500">
                      {t}
                    </span>
                  ))}
                </div>
